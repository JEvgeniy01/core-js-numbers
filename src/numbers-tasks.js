...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const coordX = (x2 - x1) ** 2;
  const coordY = (y2 - y1) ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multipleScal = x1 * x2 + y1 * y2;
  const multipleLength =
    Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = multipleScal / multipleLength;
...
function getFibonacciNumber(index) {
  ...
  if (index === 0) {
    ...
    let fib3 = 2;
    let currentIndex = 3;
    for (currentIndex; currentIndex < index; currentIndex += 1) {
      fib1 = fib2;
      fib2 = fib3;
      fib3 = fib1 + fib2;
    }
...
function getSumOfDigits(num) {
  ...
  const arr = toStr.split('').map((item) => Number(item));
...
function getRandomInteger(min, max) {
  const topValue = max - min;
  const randomValue = min + Math.random() * topValue;
